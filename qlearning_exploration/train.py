...
def get_actions():
    ...
    for item in html.find_all("a"):
        if "href" in item.attrs and item.attrs["href"].startswith("/") and item.attrs["href"] != "/2018/01/01/demo/" and item.text.strip():
            actions.append(item.attrs["href"] + "||" + encode_string(item.attrs["href"] + item.text))
    return actions
...
def loop():
    ...
    while iteration < 1000:
        ...
        browser.find_element_by_xpath('//a[contains(@href,"%s")]' % next_state).click()
        next_available_actions = get_actions()
